...
import { Message, Icon as Icon1, Card, Segment, Form, Button, Image, Label } from 'semantic-ui-react';
...
const Profile = props => {
    ...
    useEffect(() => {
        successNotification(nextProps.userUpdate.error);
        setModifiedUser(state => ({...state, ...updatedUser}));
    }, [updateError])
    ...
    imageUploadHandler = () => {
        this.props.uploadUserImage(this.state.selectedImage);
    }
                                        ...
                                        <Row>
                                            <Col align="center">
                                                <Segment style={{ width: 180 }}>
                                                    ...
                                                    <br />
                                                    <Button size='mini' onClick={() => this.imageInput.click()}>{user.img ? "Change" : "Add"}</Button>
